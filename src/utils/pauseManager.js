...
// (1) pause가 된 후 최소 100ms가 지나야 pause를 해제할 수 있도록 하고
// (2) pause가 해제된 후 최소 100ms가 지나야 다시 pause 할 수 있도록 하는 역할
...
export function pause(scene, type) {
  ...
  if (Date.now() - time_paused > 100 && game.scene.isActive(scene)) {
    ...
    if (type_pause === "pause") {
      // 바로 scene을 사용하지 않고, game.scene.getScene(scene)을 사용하는 이유:
      // scene이 pause되면 해당 scene의 변수, 함수 등을 사용할 수 없게 되기 때문
...
document.addEventListener("keydown", (event) => {
  if (
    ...
    const prevScene = game.scene.getScene(scene_paused);
    game.scene.resume(scene_paused);
    // veil과 문구를 없애줍니다.
    togglePauseScreen(prevScene, false);
    // pause 해제 소리를 재생합니다.
    prevScene.m_pauseOutSound.play();

    // 레벨업으로 인한 일시정지였을 경우, pause 해제 후 PlayingScene의 afterLevelUp 함수를 실행해 줍니다.
    if (type_pause === "levelup") {
      prevScene.afterLevelUp();
    }
