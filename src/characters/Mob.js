...
export default class Mob extends Physics.Arcade.Sprite {
   ...
   */
  constructor(scene, x, y, texture, animKey, mobSpeed = 50, initHp, dropRate) {
    ...
    if (texture === "mob1") {
      // mob 1만 바닥을 기준으로 움직이고 있습니다.
      // 움직일 때 중심을 기준으로 움직이지 않고 오프셋을 설정한 곳에 기준으로 움직이게 해두었습니다. 아주 미묘한 차이입니다.
    ...
    scene.events.on("update", (time, delta) => {
      // 현재 시간, FPS(Frame per Sec, 1초당 보여주는 프레임 수) 평활화(급격한 변화를 제외) 값
      this.update(time, delta);
    });
  }

  update() {
    // mob이 없을 경우의 예외처리입니다. (체력이 닳아서 없어진 상태)
    if (!this.body) return;
